...
class SymbolsProcessor:
    ...
    def process_frames_symbols(self, symbols: List[CroppedSymbol]) -> Optional[List[Reel]]:
        g_s = time.time()
        ...
        reels = [Reel.create_empty(frame=current_frame, index=index) for index in range(grid_size.x)]
...
if __name__ == '__main__':
    ...
    identifier = CorrSymbolIdentifier(
        read_expected_symbols(Path(r'C:\Users\user\PycharmProjects\slots_scrapper\files\agg_symbols'))
    )
    processor = SymbolsProcessor(identifier)
    symbols = read_cropped_symbols(Path(r'C:\Users\user\PycharmProjects\slots_scrapper\files\symbols\frame_0'))
